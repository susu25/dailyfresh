    ...
    def post(self, request):
        ...
        try:
            ...
            for sku_id in sku_ids:
                # 获取商品的信息
            # ----------悲观锁----------
            #     悲观锁：开始查询就加锁，同一时间只能有一个进程拿到锁，拿到锁才可以执行，执行完释放锁
            #     try:
...
class OrderPayView(View):
    ...
    def post(self, request):
        ...
        # 校验参数
        if not order_id:
            return JsonResponse({'red': 1, 'errmsg': '无效的订单id'})
        ...
        # 初始化
        alipay = AliPay(
            appid="2016092800618342", # 应用id
            app_notify_url=None, # 默认回调url
            app_private_key_path=os.path.join(settings.BASE_DIR, 'apps/order/app_private_key.pem'), # 自己的私钥
            alipay_public_key_path=os.path.join(settings.BASE_DIR, 'apps/order/alipay_public_key.pem'), # 支付宝公钥
            sign_type="RSA2", # RSA2 或者 RSA
            debug=True # 默认False,访问支付宝真实地址；True时访问支付宝沙箱
        )
